			...
			Ext.iterate(o, function (key, item) {
				...
				Ext.each(empty ? key : item, function (val) {
					buf.push(
						...
						e(key),
						'=',
						!Ext.isEmpty(val) && (val != key || !empty) ? (Ext.isDate(val) ? Ext.encode(val).replace(/"/g, '') : e(val)) : ''
					);
				});
			});
			if (!pre) {
	...
	Ext.lib.Anim = (function () {
		...
		return {
			...
			run: function (el, args, duration, easing, cb, scope, type) {
				...
				for (var k in args) {
					switch (
						k // jquery doesn't support, so convert
					) {
						case 'points':
							var by, pts;
							e.position();
						...
						case 'width':
							...
							if (args.width.from) e.setWidth(args.width.from);
							break;
						case 'height':
							o.height = args.height.to;
							if (args.height.from) e.setHeight(args.height.from);
							break;
						case 'opacity':
							o.opacity = args.opacity.to;
							if (args.opacity.from) e.setOpacity(args.opacity.from);
							break;
						case 'left':
							o.left = args.left.to;
							if (args.left.from) e.setLeft(args.left.from);
							break;
						case 'top':
							o.top = args.top.to;
							if (args.top.from) e.setTop(args.top.from);
							break;
						...
						default:
							o[k] = args[k].to;
							if (args[k].from) e.setStyle(k, args[k].from);
							break;
					}
				}
