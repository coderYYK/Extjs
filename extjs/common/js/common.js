  ...
  function checkStorage() {
    ...
    if (!window.localStorage) {
      Ext.MessageBox.alert(
        ...
        "This browser does NOT support localStorage,</br>" +
    ...
    getAll: function () {
      if (checkStorage()) {
        ...
        for (var i = 0; i < storage.length; i++) {
          ...
          allMsg +=
            storage.key(i) + " : " + storage.getItem(storage.key(i)) + "</br>";
        }
...
Ext.BLANK_IMAGE_URL =
  "/mycim2/vendor/extjs/resources/images/default/tree/s.gif";
...
Ext.Ajax.on("beforerequest", function (conn, options, eOpts) {
  if (
    options == null ||
    options.proxy == null ||
    options.proxy.queryMask == null ||
    options.proxy.queryMask
  ) {
  ...
  onComplete: function (request, xdrResult) {
    ...
    if (success) {
      ...
      if (responseData.head.code == "10000") {
        ...
        success = false;
        if (
          !responseData.head.subMessage ||
          responseData.head.subMessage == "N/A"
        ) {
          msg =
            "[" +
      ...
      this.tip = new Ext.ToolTip({
        ...
        listeners: {
          beforeshow: function updateTipBody(tip) {
            ...
            var tipText =
              tip.triggerElement.innerText || tip.triggerElement.textContent;
