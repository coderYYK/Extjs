...
function doSearch(id, type, value, path) {
  ...
  if (
    value != null &&
    value != "" &&
    trim(replace(value, "|", "", false, false)) != ""
  ) {
...
function doSearchWithExt(id, type, value, path, callback) {
  ...
  if (callback == undefined || callback == null || callback == "") {
    dest +=
      "&targetIds=" + id + "&type=" + UrlEncode(type) + "&keyValue=" + value;
  } else {
    dest +=
      "&targetIds=" +
      id +
...
function replace(target, oldTerm, newTerm, caseSens, wordOnly) {
  ...
  while ((ind = wk.indexOf(oldTerm, next)) >= 0) {
    ...
    wk =
      wk.substring(0, ind) +
      newTerm +
      wk.substring(ind + oldTerm.length, wk.length);
...
function transaction(obj, validated, updateAction) {
  ...
  if (
    sType == "novalidate" ||
    (sType == "validate" && validate(document.forms[0]) != false)
  ) {
    for (i = 0; i < document.forms[0].elements.length; i++) {
      if (
        document.forms[0].elements[i].type == "button" ||
    ...
    if (updateAction) {
      if (document.forms[0].action.indexOf("?") > 0) {
        document.forms[0].action =
          document.forms[0].action + "&action=" + obj.name;
      } else {
        document.forms[0].action =
          document.forms[0].action + "?action=" + obj.name;
      }
  ...
  if (
    ...
    for (i = 0; i < document.forms[0].elements.length; i++) {
      if (document.forms[0].elements[i].type == "submit") {
        document.forms[0].action =
          document.forms[0].action + "?" + document.forms[0].elements[i].name;
        document.forms[0].submit();
        break;
      }
    }
...
function getElementsByClassName(fatherId, tagName, className) {
  ...
  for (var i = 0, j = classNameLength; i < j; i++) {
    //创建匹配类名的正则
    className[i] = new RegExp(
      "(^|\\s)" + className[i].replace(/\-/g, "\\-") + "(\\s|$)"
    );
  }
...
function OpenOtherFrameMaskDiv() {
  if (
    window.parent != null &&
    window.parent.frames["mycim2_navi"] != null &&
    window.parent.frames["mycim2_navi"].document.readyState == "complete"
  ) {
  ...
  if (
    window.parent != null &&
    window.parent.frames["mycim2_menu"] != null &&
    window.parent.frames["mycim2_menu"].document.readyState == "complete"
  ) {
...
function checkSpecialChar(name) {
  ...
  for (var i = 0; i < str.length; i++) {
    if (iChars.indexOf(str.charAt(i)) != -1) {
      showAlertByExt(
        i18n.labels.LBS_WARNING,
        "Cannot contain half-width character &",
        2
      );
...
function searchSubmit() {
  ...
  var url =
    arguments.length > 0 && arguments[0].length > 0
      ? arguments[0]
      : doSearchUrl;
...
function addDoSearch4baseLotInfo() {
  ...
  var id =
    (arguments.length > 4 ? parseCallback(arguments[4], path) : "") +
    arguments[1];
  var $telescope = $("<a></a>");
...
function parseCallback(callback, path) {
  ...
  if (callback != void 0 && callback.length > 0) {
    ...
    var param = callback
      .substring(callback.indexOf("(") + 1, callback.lastIndexOf(")"))
      .trim();
    if (param.length > 0) {
...
function formateDueDate4lotbaseInfo($inputDueDateName) {
  ...
  var $displayDueDate = $(
    "<input type='text' class='myreadonly'  size = '32' readonly/>"
  );
  var text =
    $inputDueDateName != null ? formatDueDate($inputDueDateName.val()) : "";
 ...
 * @returns {boolean}
 */
function disabledBtnAtSubmit4Validate(
  button,
  noValidated,
  updateAction,
  needConfirm,
  ...
  if (addButtonName) {
    ...
    if (document.forms[0].action.indexOf("?") > 0) {
      document.forms[0].action =
        document.forms[0].action + "&" + button.name + "=" + value;
    } else {
      document.forms[0].action =
        document.forms[0].action + "?" + button.name + "=" + value;
    }
