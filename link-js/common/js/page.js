(function ($, window, document, undefined) {
    ...
    function Paging(element, options) {
        ...
        this.options = {
            ...
            showOne: options.showOne || 0,//只有一页时，是否显示。false=不显示,true=显示（默认）
            buttonNum: (options.buttonNum >= 5 ? options.buttonNum : 5) || 7,// 页面显示页码数量
            callback: options.callback, // 回调函数
            nowPageSize: options.nowPageSize || 20, // 查询数量
            pageSize: options.pageSize || [20, 40, 60, 80, 100]
        };
        this.init();
    }
    ...
    Paging.prototype = {
        ...
        createHtml: function () {
            ...
            var nowPageSize = this.options.nowPageSize;
            ...
            // pageSize
            content.push("<li class='xl-pageSize'><select>");
            $.each(pageSize, function (index, value) {
                if (value == nowPageSize) {
                    content.push("<option value='" + value + "' selected>" + value + "</option>");
                } else {
                    content.push("<option value='" + value + "'>" + value + "</option>");
                }
            })
            content.push("</select></li>");
            content.push("<li class='xl-jumpText xl-disabled'>条/页</li>");
            content.push("</ul>");
        ...
        bindClickEvent: function () {
            ...
            me.element.on('click', 'li', function () {
                ...
                var nowPageSize = $('.xl-pageSize option:selected').val();
                var refresh = true;
                ...
                } else if (cla === 'xl-pageSize') {
                    refresh = false;
                    if (me.options.nowPageSize != nowPageSize) {
                        me.options.nowPageSize = nowPageSize;
                        refresh = true;
                    }
                } else {
                ...
                if (refresh) {
                    ...
                    if (me.options.callback) {
                        me.options.callback(me.options.nowPage, me.options.nowPageSize);
                    }
                }
